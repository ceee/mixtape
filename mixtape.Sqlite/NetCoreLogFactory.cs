...
public class NetCoreLogFactory(ILoggerFactory loggerFactory) : ILogFactory
{
    ...
    public ILog GetLogger(Type type)
    {
        try
        {
            return new NetCoreLog(_loggerFactory.CreateLogger(type));
        }
        catch (ObjectDisposedException)
        {
            ...
            try
            {
                _loggerFactory = FallbackLoggerFactory;
                return new NetCoreLog(_loggerFactory.CreateLogger(type));
            }
            catch (ObjectDisposedException)
            {
    ...
    public ILog GetLogger(string typeName)
    {
        try
        {
            return new NetCoreLog(_loggerFactory.CreateLogger(typeName));
        }
        catch (ObjectDisposedException)
        {
            if (FallbackLoggerFactory == null) throw;
...
public sealed class NetCoreLog(ILogger logger) : ILog, ILogTrace
{
